...
function UserInfo() {
  const {user:{avatar_url, name, html_url, company, blog, location:locationInfo, email, hireable, bio, public_repos, public_gists, followers, following, created_at, updated_at}, loading, getUser} = useGithubUserStore()
  ...
  const onClickNavigateToList = useCallback(() => {
    if (!location.state) navigate('/')
      ...
      navigate({
        pathname: '/',
        search: !!location.state?.previous ? `q=${location.state?.q}&page=${location.state?.previous}` : `q=${location.state?.q}`
      })
    }
  }, [location.state, navigate])

  if (loading) {
    return <CircularProgress sx={{marginLeft: 'auto', marginTop: '200px'}} />
  } else {
    return (
      <>
        <Button
        ...
        <Card variant='outlined' sx={{margin:'10px'}}>
          <CardContent sx={{textAlign:'center'}}>
            ...
            {blog ? <Typography variant='subtitle1'>Blog : <Link href={blog}>{blog}</Link></Typography> : null}
            ...
            <Typography variant='subtitle1'>Public Gist 개수 : {public_gists}</Typography>
             <Typography variant="subtitle1">Followers 수 : {followers}</Typography>
            <Typography variant="subtitle1">Following 수 : {following}</Typography>
            <Typography variant="subtitle1">Github 생성일 : {dayjs(created_at).format("YYYY.MM.DD h:mm A")}</Typography>
            <Typography variant="subtitle1">최근 Github 업데이트 시간 :{" "}{dayjs(updated_at).format("YYYY.MM.DD h:mm A")}</Typography>
          </CardContent>
        </Card>
      </>
    )
