...
function App() {
  ...
  const [cartItems, setCartItems] = useState(JSON.parse(localStorage.getItem('cartState')) ?? [])
  ...
  return (
    <div className="relative min-h-screen">
        ...
        <section id="product-list">
          ...
          >
            {productItems.length === 0 ? (
              ...
              <ProductList productItems={productItems} toggleCart={toggleCart} setCartItems={setCartItems}/>
            )}
          </div>
        </section>
      </div>
      ...
      <aside className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
        <section
          className={`pointer-events-auto w-screen max-w-md transition ease-in-out duration-500 ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}
          id="shopping-cart"
        >
            ...
            <div className="border-t border-gray-200 p-6">
              <div className="flex justify-between font-medium">
                ...
                <p className="font-bold" id="total-count">
                  {cartItems.reduce((acc, cur) => cur.price * cur.count + acc, 0).toLocaleString('ko-KR')}원
                </p>
              </div>
              <a
                id="payment-btn"
